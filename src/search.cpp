  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
    ...
    if (!rootNode)
    {
        ...
        if (Signals.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply) || ss->ply >= MAX_PLY)
            return ss->ply >= MAX_PLY && !inCheck ? evaluate(pos).value
    ...
    else if (ttHit)
    {
        ...
        {
            EvalValue ev = evaluate(pos);
            earlyEval = ev.earlyEval;
            eval = ss->staticEval = ev.value;
        }
    ...
    else
    {
        if (ttHit)
        {
            ...
            if ((ss->staticEval = bestValue = tte->eval()) == VALUE_NONE)
                ss->staticEval = bestValue = evaluate(pos).value;
            ...
            (ss-1)->currentMove != MOVE_NULL ? evaluate(pos).value
