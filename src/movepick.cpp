...
void MovePicker::score() {
  ...
  for (auto& m : *this)
      if (Type == CAPTURES)
          ...
          m.value = ( 94 * (*mainHistory)[pos.side_to_move()][from_to(m)]
                   + 103 * (*continuationHistory[0])[pos.moved_piece(m)][to_sq(m)]
                   +  98 * (*continuationHistory[1])[pos.moved_piece(m)][to_sq(m)]
                   + 100 * (*continuationHistory[3])[pos.moved_piece(m)][to_sq(m)]
                   +  52 * (*continuationHistory[5])[pos.moved_piece(m)][to_sq(m)]) / 100;
