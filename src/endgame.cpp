...
ScaleFactor Endgame<KBPsK>::operator()(const Position& pos) const {
  ...
  if (   pos.non_pawn_material(weakSide) == BishopValueMg
      && opposite_colors(bishopSq, pos.square<BISHOP>(weakSide))
      && pos.count<PAWN>(strongSide) - pos.count<PAWN>(weakSide) <= 3)
  {
      const Square* wl = pos.squares<PAWN>(weakSide);
      Square s1, s2;
      while ((s1 = *wl++) != SQ_NONE)
      {
          //If a weak pawn is on the same color square as the strong bishop or if it is under attack, no scaling.
          if (!opposite_colors(s1, bishopSq) || (pos.attackers_to(s1) & pos.pieces(strongSide) & ~pos.pieces(weakSide)) > 0)
              return SCALE_FACTOR_NONE;
      }
      const Square* pl = pos.squares<PAWN>(strongSide);
      while ((s2 = *pl++) != SQ_NONE)
      {
          if ((forward_bb(strongSide, s2) & pos.pieces(strongSide, PAWN)))
          {
              if (!(pawns & ~file_bb(pawnsFile)))
                  continue; // there is a strong pawn further up the file and all pawns are on the same file, skip this one in the evaluation
              else
                  return SCALE_FACTOR_NONE;
          }

          Square blockSq = s2 + pawn_push(strongSide);
